(function () {
    ...
    async function handleClick(event) {
        ...
        const activateProduct = event.target.closest("[data-activate-product]"); if (activateProduct && confirm("Enable this product on the storefront?")) { try { await API.post(`/api/products/${activateProduct.dataset.activateProduct}/activate`); toast("Product enabled."); await loadCatalog(); } catch (error) { toast(error.message, "error"); } return; }
        const deleteProduct = event.target.closest("[data-delete-product]"); if (deleteProduct && confirm("Permanently delete this product and all its variants/images? This action cannot be undone.")) { try { await API.delete(`/api/products/${deleteProduct.dataset.deleteProduct}/permanent`); toast("Product permanently deleted."); await loadCatalog(); } catch (error) { toast(error.message, "error"); } return; }
        ...
        const activateVariant = event.target.closest("[data-activate-variant]"); if (activateVariant && confirm("Enable this product option?")) { const [variantId, productId] = activateVariant.dataset.activateVariant.split(":"); try { await API.post(`/api/variants/${variantId}/activate`); toast("Variant enabled."); await openVariants(productId); } catch (error) { toast(error.message, "error"); } return; }
        const deleteVariant = event.target.closest("[data-delete-variant]"); if (deleteVariant && confirm("Permanently delete this product option? This action cannot be undone.")) { const [variantId, productId] = deleteVariant.dataset.deleteVariant.split(":"); try { await API.delete(`/api/variants/${variantId}/permanent`); toast("Variant permanently deleted."); await openVariants(productId); } catch (error) { toast(error.message, "error"); } return; }
